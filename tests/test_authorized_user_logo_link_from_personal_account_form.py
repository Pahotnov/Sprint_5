...
def test_authorized_user_logo_link_from_personal_account_form(login):
    ...
    # Явное ожидание загрузки страницы "Соберите бургер" и кнопки "Оформить заказ" вместо "Войти в аккаунт" с явным ожиданием этой кнопки
    assert WebDriverWait(login, 3).until(
        expected_conditions.visibility_of_element_located(MainPageLocators.ASSEMBLE_BURGER_BLOCK)) and WebDriverWait(login, 3).until(
